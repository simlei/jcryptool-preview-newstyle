...
object testGraph extends GraphSpec:
  ...
  override type NodeT = SoleNode1.type // <-- this line makes it work, but is useless for application, since it's singleton
end testGraph
...
object testValidation extends ValidationSpec[testGraph.type, testDomain.type]:
  ...
  override type TypeOf[NT <: testDomain.baseGraph.NodeT] = NT match
...
object program:
  ...
  def main(args: Array[String]) = 
    // val x = testSetter[SoleNode1.type](SoleNode1, Right("test-should-fail")) // should fail, complaining that expected: BigInt, found: String
    val x = testSetter[SoleNode1.type](SoleNode1, Right(BigInt(1))) // should work
